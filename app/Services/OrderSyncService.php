...
class OrderSyncService
{
    ...
    public function sync(int $limit = 50, string $status = 'any'): array
    {
        ...
        try {
            $filters = [];
            if ($syncJob->last_run) {
                $filters['updated_at_min'] = $syncJob->last_run->copy()->subMinutes(2)->toIso8601String();
            }
            ...
            do {
                $pages++;
                $result = $this->shopifyService->getOrders($limit, $status, $pageInfo, $pageInfo ? [] : $filters);
                $batch = $result['orders'] ?? [];
            ...
            $this->activityTracker->setTotal(count($orders));
     ...
     *
     * @return array{pushed: int, errors: int}
     */
    ...
    public function syncOne(ShopifyOrder $order): array
    {
        ...
        if ($this->shopifyService->isConfigured() && filled($order->shopify_order_id)) {
            try {
                $remote = $this->shopifyService->getOrderDetails((string) $order->shopify_order_id);
                if ($remote !== []) {
                    $shopifyCancelled = filled($remote['cancelled_at'] ?? null) || filled($remote['cancel_reason'] ?? null);
                    if ($shopifyCancelled && $order->fulfillment_status !== 'cancelled') {
                        $this->lifecycle->markCancelled(
                            $order,
                            (string) ($remote['cancel_reason'] ?? 'Shopify iptali')
                        );
                        $this->activityTracker->log('warning', $order->order_number.' Shopify üzerinde iptal.');
                    }
                    $order->update(['synced_at' => now()]);
                    $this->activityTracker->log('info', $order->order_number.' Shopify kaydı kontrol edildi (lokal durum korundu).');
                }
            } catch (Throwable $e) {
                $errors++;
    ...
    /**
     * Existing local orders keep status / invoice / notes; only Shopify cancel is applied.
     *
     * @param  array<string, mixed>  $payload
     * @return array{order: ShopifyOrder, created: bool, cancelled: bool, skipped: bool}
     */
    private function touchExistingOrder(ShopifyOrder $existing, array $payload, bool $wasCancelled): array
    {
        ...
        $statusChanged = $mappedStatus !== (string) $existing->fulfillment_status;

        $customerInfo = $this->extractCustomerInfo($payload);
        $addressChanged = ($customerInfo['address'] !== null && $customerInfo['address'] !== (string) $existing->shipping_address)
            || ($customerInfo['province'] !== null && $customerInfo['province'] !== (string) $existing->shipping_province)
            || ($customerInfo['zip'] !== null && $customerInfo['zip'] !== (string) $existing->shipping_zip);

        $existing->update(array_filter([
            'synced_at' => now(),
            ...($statusChanged ? ['fulfillment_status' => $mappedStatus] : []),
            ...($customerInfo['address'] ? ['shipping_address' => $customerInfo['address']] : []),
            ...($customerInfo['city'] ? ['shipping_city' => $customerInfo['city']] : []),
            ...($customerInfo['province'] ? ['shipping_province' => $customerInfo['province']] : []),
            ...($customerInfo['zip'] ? ['shipping_zip' => $customerInfo['zip']] : []),
            ...($customerInfo['phone'] && blank($existing->customer_phone) ? ['customer_phone' => $customerInfo['phone']] : []),
        ], static fn ($v) => $v !== null));
        ...
        return [
            'order' => $existing->fresh() ?? $existing,
            'created' => false,
            'cancelled' => $justCancelled,
            'skipped' => ! $statusChanged && ! $justCancelled && ! $addressChanged,
        ];
    }
    ...
    private function mapInboundShopifyStatus(ShopifyOrder $existing, string $incoming): string
    {
        ...
        if ($incoming === 'cancelled' || $incoming === 'restocked') {
            return 'cancelled';
        }
